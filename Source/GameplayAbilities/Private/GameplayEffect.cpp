...
bool UGameplayEffect::CanApply(const FActiveGameplayEffectsContainer& ActiveGEContainer, const FGameplayEffectSpec& GESpec, FPredictionKey PredictionKey) const
{
	for (const UGameplayEffectComponent* GEComponent : GEComponents)
	{
		if (GEComponent && !GEComponent->CanGameplayEffectApply(ActiveGEContainer, GESpec, PredictionKey))
		{
...
void FActiveGameplayEffect::PreReplicatedRemove(const struct FActiveGameplayEffectsContainer &InArray)
{
	...
	// Change the global path to avoid repeting things.. 
...
void FActiveGameplayEffectsContainer::InternalOnActiveGameplayEffectRemoved(FActiveGameplayEffect& Effect, bool bInvokeGameplayCueEvents, const FGameplayEffectRemovalInfo& GameplayEffectRemovalInfo)
{
	...
	//bool bPredictiveRemove = Effect.bRemovedPredictively;
	const bool bPredictiveRemove = Effect.PredictiveRemovalKey.IsValidKey();
	if (Effect.Spec.Def)
	{
		// Only Need to update tags and modifiers if the gameplay effect is active.
		if (!Effect.bIsInhibited && !bPredictiveRemove)
		{
...
const FActiveGameplayEffect* FActiveGameplayEffectsContainer::GetPredictedEffectWithPredictionKey(FPredictionKey PredictionKey, FActiveGameplayEffect* CompareEffect, bool bCreatedLocally) const
{
	for (const FActiveGameplayEffect& Effect : this)
	{
		if (CompareEffect && *CompareEffect == Effect)
		{
			continue;
		}

		if (Effect.PredictionKey == PredictionKey)
		{
			if (bCreatedLocally && Effect.PredictionKey.WasReceived())
			{
				continue;
			}
