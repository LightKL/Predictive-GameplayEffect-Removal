...
namespace UE::AbilitySystem::Private
{
	...
	// 0x1 and 0x2 should be inversed as 0x1 can't be compatible with 0 (same bit) but nothing is wrong with having (old implying news) & (news implying old) enabled.
	...
	FAutoConsoleVariableRef CVarDependentChainBehavior(TEXT("AbilitySystem.PredictionKey.DepChainBehavior"), CVarDependentChainBehaviorValue,
		TEXT("How do we handle dependency key chains? Bitmask: 0 = Old Accept/Rejected Implies Newer Accepted/Rejected. 0x1 = Old Accepted Does NOT imply Newer Accepted. 0x2 = Newer Accepted also implies Older Accepted"));
...
bool FPredictionKey::NetSerialize(FArchive& Ar, class UPackageMap* Map, bool& bOutSuccess)
{
	...
	{
		// Serialize the DependentChainBehavior as uint8 (handled by FArchive)
		uint8 ChainBehaviorValue = static_cast<uint8>(DependentChainBehavior);
		Ar << ChainBehaviorValue;
		if (Ar.IsLoading())
		{
			DependentChainBehavior = static_cast<EDependentChainBehavior>(ChainBehaviorValue);
		}
...
void FPredictionKey::GenerateDependentPredictionKey()
{
	...
	if (Previous > 0)
	{
		FPredictionKeyDelegates::AddDependency(Current, Previous, (int32)DependentChainBehavior);
	}
}

FPredictionKey FPredictionKey::CreateNewPredictionKey(const UAbilitySystemComponent* OwningComponent, EDependentChainBehavior NewDependentChainBehavior)
{
	...
	if(OwningComponent->GetOwnerRole() != ROLE_Authority)
	{
		...
		NewKey.DependentChainBehavior = NewDependentChainBehavior;
	}
	return NewKey;
}
...
void FPredictionKeyDelegates::AddDependency(FPredictionKey::KeyType ThisKey, FPredictionKey::KeyType DependsOn, int32 DependentChainBehavior)
{
	...
	if ((DependentChainBehavior & 1) == 0)
	{
		NewCaughtUpDelegate(DependsOn).BindStatic(&FPredictionKeyDelegates::CatchUpTo, ThisKey);
	}
	else
	{
		// Base key being accepted does not imply New Key accepted
		// Use the AbilitySystem Log for this one.
		UE_LOG(LogTemp, Warning, TEXT("Current key : [%d] won't caught up when the base key [%d] will. Because of these flags : [%d] on EDependentChainBehavior"), ThisKey, DependsOn, DependentChainBehavior);
	}
...
FScopedPredictionWindow::FScopedPredictionWindow(UAbilitySystemComponent* InAbilitySystemComponent, bool bCanGenerateNewKey, EDependentChainBehavior NewDependentChainBehavior)
{
	...
	if (bCanGenerateNewKey)
	{
		...
		InAbilitySystemComponent->ScopedPredictionKey.DependentChainBehavior = NewDependentChainBehavior;
