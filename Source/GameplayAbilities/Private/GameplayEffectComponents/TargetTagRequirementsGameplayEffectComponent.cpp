...
bool UTargetTagRequirementsGameplayEffectComponent::OnActiveGameplayEffectAdded(FActiveGameplayEffectsContainer& GEContainer, FActiveGameplayEffect& ActiveGE) const
{
	...
	{
		// Why ? Because if Removal GE is rejected -> We are OFF and we can't get back ON
		...
		if (bRemovalRequirementsMet)
		{
			...
			for (const FGameplayTag& Tag : GameplayTagsToBind)
			{
				...
				FDelegateHandle Handle = OnTagEvent.AddUObject(this, &UTargetTagRequirementsGameplayEffectComponent::OnRemovalGameplayTagChange, ASC, ActiveGE.Handle);
		...
		{
			// Otherwise just enable back the GE if no removal tags are present
...
void UTargetTagRequirementsGameplayEffectComponent::OnRemovalGameplayEffectRemoved(const FGameplayEffectRemovalInfo& GERemovalInfo, UAbilitySystemComponent* ASC, TTuple<FGameplayTag, FDelegateHandle> BoundEvent) const
{
...
void UTargetTagRequirementsGameplayEffectComponent::OnGameplayEffectRemoved(const FGameplayEffectRemovalInfo& GERemovalInfo, UAbilitySystemComponent* ASC, TArray<TTuple<FGameplayTag, FDelegateHandle>> AllBoundEvents) const
{
	...
	// This OnEffectRemoved can be caused by Removal GE removing us locally.
	// we need to have a way, when getting enabled, in case Removal GE is rejected
...
void UTargetTagRequirementsGameplayEffectComponent::OnRemovalGameplayTagChange(const FGameplayTag GameplayTag, int32 NewCount, UAbilitySystemComponent* ASC, FActiveGameplayEffectHandle ActiveGEHandle) const
{
	if (NewCount == 0)
	{
		// Removal GE is now removed. 
		// If predictive Removal GE is removed, it would trigger tag delegates; 
		...
		if (ensure(ASC))
		{
			...
			if (ActiveEffect)
			{
				// Removal GE is removed. It will either be rejected, nothing happens, we 
				// successfully predicted removal
				// if Removal GE is confirmed, we will get removed anyways. We could have a small
	...
	{
		// Removal GE is re-applied due to replication. This means Removal GE has been CaugthUp
	...
	if (ActiveGE)
	{
		...
		if (bRemovalRequirementsMet)
		{
			...
			else if (Owner->CanPredict())
			{
				...
				// Register Removal GE tag events to handle different use cases
				FDelegateHandle DelegateHandle = Owner->RegisterGameplayTagEvent(GameplayTag).AddUObject(this, &ThisClass::OnRemovalGameplayTagChange, Owner, ActiveGEHandle);
				TTuple<FGameplayTag, FDelegateHandle> BoundEvent = { GameplayTag, DelegateHandle };

				// Unregister those when the effect is removed
				ActiveGERef.EventSet.OnEffectRemoved.AddUObject(this, &ThisClass::OnRemovalGameplayEffectRemoved, Owner, MoveTemp(BoundEvent));
			}
		}
		else
		{
