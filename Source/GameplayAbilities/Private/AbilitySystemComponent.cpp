...
void UAbilitySystemComponent::EnableGameplayEffect(FActiveGameplayEffect& ActiveEffect)
{
	...
	// Set the removal key to null (Current : 0) to be considered as invalid
	ActiveEffect.PredictiveRemovalKey = FPredictionKey();
	
	// The magic call to add tags & mods and invoke cues
	constexpr bool bInvokeGameplayCueEvents = true;
	ActiveGameplayEffects.AddActiveGameplayEffectGrantedTagsAndModifiers(ActiveEffect, bInvokeGameplayCueEvents);

	// This is a way to notify that OnEffectRemoved has failed
	FPredictionKeyDelegates::Reject(RemovalKey.Current);
}

void UAbilitySystemComponent::DisableGameplayEffect(FActiveGameplayEffect& ActiveEffect)
{
	// Scope locks for the AGE array and the OnDirty aggregators calls 
	FScopedActiveGameplayEffectLock ScopeLockActiveGameplayEffects(ActiveGameplayEffects);
	FScopedAggregatorOnDirtyBatch	AggregatorOnDirtyBatcher;
	...
	// dependencies. Creating a dependent key would be a solution for this
	ActiveEffect.PredictiveRemovalKey = FPredictionKey::CreateNewPredictionKey(this);

	// The magic call to remove tags & mods and invoke cues
	constexpr bool bInvokeGameplayCueEvents = true;
	ActiveGameplayEffects.RemoveActiveGameplayEffectGrantedTagsAndModifiers(ActiveEffect, bInvokeGameplayCueEvents);

	// Fire events
	FGameplayEffectRemovalInfo RemovalInfo;
	RemovalInfo.ActiveEffect = &ActiveEffect;
	
	// We only predict forced removal via GEs (not natural expiration) for now.
	constexpr bool bRemovedPrematurely = true;
	RemovalInfo.bPrematureRemoval = bRemovedPrematurely;

	RemovalInfo.StackCount = ActiveEffect.Spec.GetStackCount();
	RemovalInfo.EffectContext = ActiveEffect.Spec.GetContext();
	ActiveEffect.EventSet.OnEffectRemoved.Broadcast(RemovalInfo);

	OnAnyGameplayEffectRemovedDelegate().Broadcast(ActiveEffect);
}

FActiveGameplayEffect* UAbilitySystemComponent::GetActiveEffectWithPredictionKey(const FPredictionKey& PredictionKey) const
{
	// We grab the active gameplay effect sharing the same prediction key
	constexpr bool bCreatedLocally = false;
	const FActiveGameplayEffect* ActiveEffect = ActiveGameplayEffects.GetPredictedEffectWithPredictionKey(PredictionKey, nullptr, bCreatedLocally);
	if (ActiveEffect)
	{
		return const_cast<FActiveGameplayEffect*>(ActiveEffect);
	}
...
FActiveGameplayEffectHandle UAbilitySystemComponent::SetActiveGameplayEffectInhibit(FActiveGameplayEffectHandle&& ActiveGEHandle, bool bInhibit, bool bInvokeGameplayCueEvents)
{
	...
	if (ActiveGE->bIsInhibited != bInhibit)
	{
		...
		/*if (ActiveGE->bRemovedPredictively)
		{
			return MoveTemp(ActiveGEHandle);
		}*/

		// For now, just check if the key is valid.
		if (ActiveGE->PredictiveRemovalKey.IsValidKey())
		{
			return MoveTemp(ActiveGEHandle);
		}
...
FActiveGameplayEffectHandle UAbilitySystemComponent::ApplyGameplayEffectSpecToSelf(const FGameplayEffectSpec &Spec, FPredictionKey PredictionKey)
{
	...
	if (!Spec.Def->CanApply(ActiveGameplayEffects, Spec, PredictionKey))
	{
		return FActiveGameplayEffectHandle();
	}
