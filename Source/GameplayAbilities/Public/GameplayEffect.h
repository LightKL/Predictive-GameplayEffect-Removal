...
struct GAMEPLAYABILITIES_API FActiveGameplayEffect : public FFastArraySerializerItem
{
	...
	/** Tells us if we are predictively removed */
	bool IsPredictivelyRemoved() const
	{
		return PredictiveRemovalKey.IsLocalClientKey();
	}
...
struct GAMEPLAYABILITIES_API FActiveGameplayEffectsContainer : public FFastArraySerializer
{
	...
	const FActiveGameplayEffect* GetPredictedEffectWithPredictionKey(FPredictionKey PredictionKey, FActiveGameplayEffect* CompareEffect, bool bCreatedLocally) const;
...
public:
	 ...
	 */
	bool CanApply(const FActiveGameplayEffectsContainer& ActiveGEContainer, const FGameplayEffectSpec& GESpec, FPredictionKey PredictionKey) const;
