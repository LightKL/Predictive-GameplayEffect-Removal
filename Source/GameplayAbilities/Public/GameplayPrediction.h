...
{
	OlderAcceptRejectNewer = 0,
	OlderNotAcceptNewer = 0x1,
	NewerAcceptOlder = 0x2,
};
...
struct GAMEPLAYABILITIES_API FPredictionKey
{
	...
	UPROPERTY()
	EDependentChainBehavior DependentChainBehavior = EDependentChainBehavior::OlderAcceptRejectNewer;

	/** Construct a new prediction key with no dependencies */
	static FPredictionKey CreateNewPredictionKey(const UAbilitySystemComponent*, EDependentChainBehavior NewDependentChainBehavior = EDependentChainBehavior::OlderAcceptRejectNewer);
...
public:
	...
	static void AddDependency(FPredictionKey::KeyType ThisKey, FPredictionKey::KeyType DependsOn, int32 DependentChainBehavior);
...
struct GAMEPLAYABILITIES_API FScopedPredictionWindow
{
	...
	FScopedPredictionWindow(UAbilitySystemComponent* AbilitySystemComponent, bool CanGenerateNewKey = true, EDependentChainBehavior NewDependentChainBehavior = EDependentChainBehavior::OlderAcceptRejectNewer);
