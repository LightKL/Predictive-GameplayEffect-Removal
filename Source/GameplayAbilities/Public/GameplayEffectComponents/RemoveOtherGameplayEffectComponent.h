...
struct FActiveGEHandleWithPredictionKey
{
public:
	FActiveGEHandleWithPredictionKey()
	{
	}

	FActiveGEHandleWithPredictionKey(FActiveGameplayEffectHandle NewHandle, const FPredictionKey& NewKey)
		: Handle(NewHandle), PredictionKey(NewKey)
	{
	}
...
protected:
	...
	void OnGameplayEffectRemovalFailed(UAbilitySystemComponent* OwnerASC, TArray<FActiveGEHandleWithPredictionKey> RemoveThoseGEs) const;
